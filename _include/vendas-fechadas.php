<?php if(isset($_POST['inclui'])){ if(isset($_SESSION)==false){session_start();}
		...
		include('_classes/Cadastros.class.php');
			...
			include($acres."_classes/FrenteCaixa.class.php");
			$trasData=FrenteCaixa::DataCaixaFechado($user_id_empresa);
			...
			$count=FrenteCaixa::countCaixasFechados($user_id_empresa,$dataInicial,$dataFinal);
        if($count['count']>0){
    ?>
<div class="d_volta_admin"><a href="<?php echo getUrl(); ?>administracao.php?relatorio=frente-caixa">Voltar para início de Administração</a></div>
    <div id="tudo_cmd_aberta">    
     <div class="info_topo_cad"><h1>Caixas fechados</h1></div><!--fecha info topo cad-->    
     <?php 
	$pgOndefiltra='vendas-fechadas';
	...
	include($acres.'_include/filtro.php'); 
      ...
      echo '<div id="tudoCarregaCmd">';
        if($qualAtivo=="busca"){
			...
			$cmdFechadas=FrenteCaixa::buscaCaixaFechadas($_GET["busca"],$user_id_empresa);
		...
		$cmdFechadas=FrenteCaixa::trazerCaixasFechadas($user_id_empresa,$inicio,$maximo,$dataInicial,$dataFinal);
        $numCmd=$cmdFechadas['num'];
        }
        if($numCmd>0){
		$qualCad='vendas-fechadas';
		$qualvai=$qualAtivo;
    ...
    <h1>sem vendas fechados !</h1>
    </div>
<div class="d_volta_admin"><a href="<?php echo getUrl(); ?>administracao.php?relatorio=frente-caixa" id="qualPgEsta" class="frente-caixa">Voltar para início de Administração</a></div>